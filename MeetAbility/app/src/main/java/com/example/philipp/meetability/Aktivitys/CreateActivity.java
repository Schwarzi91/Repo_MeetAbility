        ...
        if(view == btCreate)
        {
            if(etLocation.length()>0){
                if(etDescription.length()>0) {
                    ...
                    participant= new Participant(LoginActivity.usercheckItem.getUser_id(), Storage.getStorageInstance().getAktivityList().get(Storage.getStorageInstance().getAktivityList().size()-1).getAktivityId(), true, 0);
